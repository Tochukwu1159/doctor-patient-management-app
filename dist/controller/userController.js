...
async function RegisterUser(req, res, next) {
    ...
    try {
        ...
        const duplicatEmail = await reportModel_1.UserInstance.findOne({ where: { email: req.body.email } });
        if (duplicatEmail) {
            return res.status(409).json({
                msg: "Email is used, please change email"
            });
        }
        const duplicatePhone = await reportModel_1.UserInstance.findOne({ where: { phonenumber: req.body.phonenumber } });
        if (duplicatePhone) {
            return res.status(409).json({
        ...
        const record = await reportModel_1.UserInstance.create({
            id: id,
...
async function LoginUser(req, res, next) {
    ...
    try {
        ...
        const User = await reportModel_1.UserInstance.findOne({ where: { email: req.body.email } });
        const { id } = User;
...
async function getUsers(req, res, next) {
    try {
        ...
        const record = await reportModel_1.UserInstance.findAndCountAll({ limit, offset, include: [
                {
                    model: doctorModel_1.TodoInstance,
                    as: 'todo'
                }
            ] });
        res.status(200).json({
