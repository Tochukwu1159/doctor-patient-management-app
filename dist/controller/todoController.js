...
async function Todos(req, res, next) {
    ...
    try {
        ...
        const record = await doctorModel_1.TodoInstance.create({
            id,
            ...req.body,
            userId: verified.id,
        });
...
async function getTodos(req, res, next) {
    try {
        ...
        const record = await doctorModel_1.TodoInstance.findAndCountAll({ limit, offset,
            include: [{
                    model: reportModel_1.UserInstance,
...
async function getSingleTodo(req, res, next) {
    try {
        const { id } = req.params;
        const record = await doctorModel_1.TodoInstance.findOne({ where: { id } });
